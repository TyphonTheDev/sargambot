module.exports = {
    ...
    description: 'Unhides The Current/Mentioned Channels for everyone',
    timeout: '',
    category: 'moderation',
    run: async (client, message, args) => {
        ...
        if (message.mentions.channels.first()) {
            await message.mentions.channels.forEach(async channel => {
                ...
                message.channel.send(`${client.emotes.success} | <#${channel.id}> Has Been Unhidden!`)
            })
        }
        else if (!args[0]) {
            ...
            message.channel.send(`${client.emotes.success} | <#${message.channel.id}> Has Been Unhidden!`)
        }
        else return;
    }
}