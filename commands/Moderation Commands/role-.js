    ...
    run: async (client, message, args) => {
        ...
        switch (args[0].toLowerCase()) {
            case `bots`:
                ...
                let memberarray1 = role1.members.array().filter(member => member.user.bot);
                ...
                const m1 = await message.channel.send(`${client.emotes.success} | Removing \`${role1.name}\` From ${membercount1} Bots.`);
                for (var i = 0; i < membercount1; i++) {
                    let member = memberarray1[i];
                    member.roles.remove(role1);
                    if (i <= membercount1) {
                        m1.edit(`${client.emotes.success} | Removed \`${role1.name}\` From ${membercount1} Bots!`)
                    }
                }
                break;
            ...
            case `humans`:
                ...
                let memberarray2 = role2.members.array().filter(member => !member.user.bot);
                ...
                const m2 = await message.channel.send(`${client.emotes.success} | Removing \`${role2.name}\` From ${membercount2} Humans!`);
                for (var i = 0; i < membercount2; i++) {
                    let member = memberarray2[i];
                    member.roles.remove(role2);
                    if (i <= membercount2) {
                        m2.edit(`${client.emotes.success} | Removed \`${role2.name}\` From ${membercount2} Humans!`)
                    }
                }
                break;
            ...
            case `all`:
                ...
                let memberarray3 = role3.members.array();
                ...
                const m3 = await message.channel.send(`${client.emotes.success} | Removing \`${role3.name}\` From ${membercount3} Users!`);
                for (var i = 0; i < membercount3; i++) {
                    let member = memberarray3[i];
                    member.roles.remove(role3);
                    if (i <= membercount3) {
                        m3.edit(`${client.emotes.success} | Removed \`${role3.name}\` From ${membercount3} Users!`)
                    }
                }
                break;
            ...
            default:
                ...
                const m4 = await message.channel.send(`${client.emotes.success} | Removed \`${roleassign.name}\` From ${message.mentions.members.map(member => '\`' + member.user.tag + '\`').join(", ")}!`);
                for (var i = 0; i < membercount4; i++) {
                    let member = memberarray4[i];
                    member.roles.remove(roleassign);
                    if (i <= membercount4) {
                        m4.edit(`${client.emotes.success} | Removed \`${roleassign.name}\` From ${message.mentions.members.map(member => '\`' + member.user.tag + '\`').join(", ")}!`)
                    }
                }
        }
    }
